...
function createDummyGame(id: number, state: GameSession["state"], players: Player[]): GameSession {
  return {
    ...
    reshuffleCount: 0,
    suddenDeath: false,
    lastActionTime: new Date(), // Add the new property
  }
}
...
Deno.test("JSONGameStore - Save and Load Game", async (t) => {
  ...
  await t.step("should save and load a game", async () => {
    const players: Player[] = [{ id: 1, firstName: "Player 1", state: "active", username: "p1", hand: [] }]
...
Deno.test("JSONGameStore - Get Player Games", async (t) => {
  ...
  await t.step("should get player games", async () => {
    const player1: Player = { id: 1, firstName: "A", state: "active", username: "a", hand: [] }
    const player2: Player = { id: 2, firstName: "B", state: "active", username: "b", hand: [] }
